...
export function UserPromptContent({
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        setError(null);
        const path = buildUserPromptFilePath(derivedUserId, sessionId, targetFile);
  ...
  const resolveNextVersionName = () => {
    ...
    const base = targetFile ?? "userprompt.txt";
  ...
  const updateSessionIndex = async (
    ...
    try {
      ...
      const updated = sessions.map((s) => {
        ...
        const files = s.user_prompt_files ?? [];
        ...
        return {
          ...s,
          user_prompt_file_name: makeActive ? newFileName : s.user_prompt_file_name,
          user_prompt_files: ordered,
        };
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      console.error("[UserPromptContent] Save failed", err);
  ...
  const handleSaveAs = async () => {
    ...
    try {
      const path = buildUserPromptFilePath(derivedUserId, sessionId, newName);
      await apiPut(path, {
        headers: { "Content-Type": "text/plain" },
        body: content,
      });
      ...
      console.error("[UserPromptContent] Save as new version failed", err);
  ...
  const handlePublish = async () => {
    ...
    try {
      ...
      console.error("[UserPromptContent] Publish failed", err);
  ...
  return (
    <div>
      <div className="mb-2 text-sm font-medium text-dark dark:text-white">User Prompt</div>
      <textarea
        value={content ?? ""}
