...
export const metadata: Metadata = {
  title: "Web Widget Editor",
};

type WebWidgetEditorSearchParams = Promise<{ session_id?: string }>;

export default async function WebWidgetEditorPage({
  searchParams,
}: {
  searchParams: WebWidgetEditorSearchParams;
}) {
    ...
    <div className="mx-auto w-full max-w-[1460px]">
      ...
      <div className="mt-6 rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h2 className="text-lg font-semibold text-dark dark:text-white">Web Widget Editor</h2>
        ...
        <WebWidgetEditor sessionId={sessionId} />
      </div>
    </div>
  );
}
